...
{
    int *array = (int *)malloc(length * sizeof(int));   // reservierung von speicher auf dem Heap
...
int **creatMatrix(unsigned int num_rows, unsigned int num_cols, int value)
{
...
void printMatrix(int **matrix, unsigned int num_rows, unsigned int num_cols)
{
    ...
    for (unsigned int i = 0; i < num_rows; i++)
    {
        for (unsigned int j = 0; i < num_cols; j++)
        {
            printf("%d\n", matrix[i][j]);
        }
